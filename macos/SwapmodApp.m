...
- (void)presentNativeFilePicker {
    ...
    [panel beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse result) {
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            NSUInteger rawProjectCount = 0;

            for (NSURL *originalURL in panel.URLs) {
                NSURL *resolvedURL = originalURL;
                NSString *lowerName = originalURL.lastPathComponent.lowercaseString;

                if (
                    [lowerName hasSuffix:@".3mf"] &&
                    ![lowerName hasSuffix:@".gcode.3mf"] &&
                    ![lowerName hasSuffix:@".swap.3mf"] &&
                ...
                [payloads addObject:@{
                    @"name": resolvedURL.lastPathComponent ?: @"import.3mf",
